...
class Arena {
 public:
  ...
  /**
   * @brief Function that adds the desired number of robots to the Arena.
   *
   * @param[in] type is the type of the entity, here it will always be robot.
   * @param[in] quantity is the number of robots to be added.
   */
  void AddRobot(EntityType type, int quantity);
  /**
   * @brief Function that adds the desired number of food to the Arena.
   *
   * @param[in] type is the type of the entity, here it will always be food.
   * @param[in] quantity is the number of food to be added.
   */
  void AddFood(EntityType type, int quantity);
  /**
   * @brief Function that adds the desired number of lights to the Arena.
   *
   * @param[in] type is the type of the entity, here it will always be lights.
   * @param[in] quantity is the number of lights to be added.
   */
  void AddLight(EntityType type, int quantity);

  /**
   * @brief Accepting communication from the controller and
   * dispatching as appropriate.
   *
   * @param[in] The communication that needs to be carried out.
   */
  ...
  *
  * @param[in] ent The entity that has to be moved to the edge of the wall.
  * @param[in] wall Denotes which wall it is - i.e. left, right etc.
  **/
  ...
  /**
   * @brief Getter for the robot vectors in the arena.
   *
  ...
  void set_robot_count(int rcount) { robot_count_ = rcount; }

  int get_light_count() { return light_count_; }
  void set_light_count(int lcount) { light_count_ = lcount; }

  int get_food_count() { return food_count_; }
  void set_food_count(int fcount) { food_count_ = fcount; }

  /**
  *
  * @brief Depending on the % of fear, makes that many robots fear light.
  * This method also informs robot whether food is on or off, and also sets the
  * light sensitivity of the robot towards light.
  *
  * 1 denotes fear behavior, 0 denotes exploratory behavior.
  * @param[in] fcount the number of robots that fear light
  * @param[in] lsensor_base the light sensitivity value specified by user
  * @param[in] fonoff the value for food on/off
  */
  void set_behavior_sensitivity_robot(int fcount, float lsensor_base,
                                                 int fonoff);
 ...
 private:
  ...
  // Vector for all the robots in the arena.
  ...
  int robot_count_;
  // number of lights in the arena
  int light_count_;
  // number of food entities in the arena
  int food_count_;
};
