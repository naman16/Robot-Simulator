...
class GraphicsArenaViewer : public GraphicsApp {
 public:
  ...
  /**
   * @brief Handle the user pressing the result button on the GUI.
   *
   */
  void OnResultBtnPressed();
  ...
  /**
   *
   * Function to deal when the game is over and the game_status_ has been changed
   * to LOST or WON.
   */
  void GameOver();
 ...
 private:
  ...
  /**
   * @brief This function is responsible for drawing a slider in the graphics
   * window so that the user can select the number of robots between 0 and 10.
   *
   * @param[in] window The graphics arena window in which everything is
   * happening
   */
  void DrawRobotSlider(nanogui::ref<nanogui::Window> window);
  /**
   * @brief This function is responsible for drawing a slider in the graphics
   * window so that the user can select the number of lights between 0 and 5.
   *
   * @param[in] window The graphics arena window in which everything is
   * happening
   */
  void DrawLightSlider(nanogui::ref<nanogui::Window> window);
  /**
   * @brief This function is responsible for drawing a slider in the graphics
   * window so that the user can select if the user wants food on or off.
   *
   * @param[in] window The graphics arena window in which everything is
   * happening
   */
  void DrawFoodOnOffSlider(nanogui::ref<nanogui::Window> window);
  /**
   * @brief This function is responsible for drawing a slider in the graphics
   * window so that the user can select the number of food entities
   * between 0 and 5.
   *
   * @param[in] window The graphics arena window in which everything is
   * happening
   */
  void DrawFoodSlider(nanogui::ref<nanogui::Window> window);
  /**
   * @brief This function is responsible for drawing a slider in the graphics
   * window so that the user can select the percentage of robots that fear
   * light.
   *
   * @param[in] window The graphics arena window in which everything is
   * happening
   */
  void DrawFearSlider(nanogui::ref<nanogui::Window> window);
  /**
   * @brief This function is responsible for drawing a slider in the graphics
   * window so that the user can select the sensitivity of the light sensors.
   *
   * @param[in] window The graphics arena window in which everything is
   * happening
   */
  void DrawLightSensorSlider(nanogui::ref<nanogui::Window> window);

  Controller *controller_;
  Arena *arena_;
  bool paused_{true};
  bool game_result_{false};
  bool game_start_{false};

  // buttons
  nanogui::Button *playing_button_{nullptr};
  nanogui::Button *newgame_result_button_{nullptr};
  ...
  int food_count_;
  // number of robots that fear light in the arena.
  int fear_count_;
  // the sensitivity of the light sensor.
  float light_sensitivity_;
  // 1 means food is on in the arena, 0 means food is off in the arena.
  int food_on_off_;
};
